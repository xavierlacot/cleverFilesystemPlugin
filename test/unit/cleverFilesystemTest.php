...
foreach ($filesystems_configs as $type => $filesystem_config)
{
  ...
  # write tests
  $fs->write('subdir/toto.txt', 'Hello, here is titi');
  $t->ok('Hello, here is titi' === $fs->read('/subdir/toto.txt'), 'write can overwrite the content of a file');

  try
  {
    $fs->write('subdir/toto.txt', 'Hello, here is Michel, and I cannot overwrite this file', false);
  }
  catch (sfException $e)
  {
    $t->pass('It is possible to forbid to overwrite files');
  }

  $fs->write('subdir/toto.txt', '. I am happy that I can append text!', true, true);
  $t->ok(
    'Hello, here is titi. I am happy that I can append text!' === $fs->read('/subdir/toto.txt'),
    'It is possible to append some content to a file'
  );
